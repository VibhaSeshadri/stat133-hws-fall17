...
#### Examples
###### Let's see some `dplyr` and `SQL` in action!
...
```{r}
# Total Male Accepted
male <- cal %>% filter(Gender == "Male") %>% filter(Admit == "Admitted") %>% select(Freq) %>% summarise(sum(Freq))

# Total Female Accepted
female <- cal %>% filter(Gender == "Female") %>% filter(Admit == "Admitted") %>% select(Freq) %>% summarise(sum(Freq))

# How many more men than women were accepted?
male - female
```

How would we do the same thing in `SQL?`
```{sql}
select count(Freq) where Gender = 'Male' and Admit = 'Admitted'; 
```
